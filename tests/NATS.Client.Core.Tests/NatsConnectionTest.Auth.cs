    ...
    public static IEnumerable<object[]> GetAuthConfigs()
    {
        yield return new object[]
        {
            new Auth(
                ...
                {
                    AuthOpts = NatsAuthOpts.Default with { Token = "s3cr3t" }
                }
            )
        };
    ...
    public async Task UserCredentialAuthTest(Auth auth)
    {
        ...
        _output.WriteLine("TRY ANONYMOUS CONNECTION");
        {
            await using var failConnection = server.CreateClientConnection(
                ignoreAuthorizationException: true
            );
            var natsException = await Assert.ThrowsAsync<NatsException>(
                async () => await failConnection.PublishAsync(subject, 0)
            );
