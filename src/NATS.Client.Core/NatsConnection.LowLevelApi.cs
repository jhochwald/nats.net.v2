...
public partial class NatsConnection
{
    ...
    /// </summary>
    internal ValueTask PubPostAsync(
        string subject,
    ...
    )
    {
        ...
        {
            var command = PublishBytesCommand.Create(
                ObjectPool,
                subject,
                replyTo,
                headers,
        ...
        return WithConnectAsync(
            subject,
            replyTo,
            headers,
            payload,
            cancellationToken,
            static (self, s, r, h, p, c) =>
            {
    ...
    internal ValueTask PubModelPostAsync<T>(
        string subject,
        T? data,
    ...
    )
    {
        ...
        if (ConnectionState == NatsConnectionState.Open)
        {
            var command = PublishCommand<T>.Create(
                ObjectPool,
                subject,
                replyTo,
                headers,
    ...
    )
    {
        ...
        if (ConnectionState == NatsConnectionState.Open)
        {
            var command = AsyncPublishBytesCommand.Create(
                ObjectPool,
        ...
        return WithConnectAsync(
            ...
            payload,
            cancellationToken,
            static (self, s, r, h, p, token) =>
            {
                var command = AsyncPublishBytesCommand.Create(
                    self.ObjectPool,
                    self.GetCancellationTimer(token),
                    s,
                    r,
                    h,
                    p
    ...
    internal ValueTask PubModelAsync<T>(
        string subject,
        T? data,
        INatsSerializer serializer,
        string? replyTo = default,
    ...
    )
    {
        ...
        if (ConnectionState == NatsConnectionState.Open)
        {
            var command = AsyncPublishCommand<T>.Create(
                ObjectPool,
                GetCancellationTimer(cancellationToken),
                subject,
                replyTo,
